...
final_currency_list <- rjson_fromJSON(file = file.path(params$app_path, "json/final_currency_inv_date.json"))
...
emna <- month(salary_list$dates$end, label = TRUE, abbr = FALSE)
...
final_currency <- final_currency_list$final_currency
salary_currency <- salary_list$main$currency
...
grouped_currency <- grouped_list$currency
...
\vspace{-12truemm}
\raggedright `r fields_names_list$invoiceNumber[lan_idx]`: `r params$invoiceNumber` \par
...
\raggedleft `r consultant_business_list$country`    
...
|--------|-------:|----|----:|----|-----------:|
|`r salary_list$main$section_title`|  |  | | | |
...
cat( # nolint
  ...
  if (use_dates) {
    paste0("|\\hspace{2mm}", salary_list$dates$delivery_month_text, " ", emna, " ", sy, "| | |  |   |   |\n")
  },
    ...
    for (grouped_idx in seq_len(nrow(grouped_df))) {
      grouped_strings <- c(
        ...
        paste0(
          ...
          " ", grouped_df[grouped_idx, "value"], "|", grouped_df[grouped_idx, "currency"], " ", "| |", "\n"
        )
      )
    }
    grouped_strings <- c(
