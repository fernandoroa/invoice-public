...
json_dates_list <- rjson::fromJSON(file = "json/input_dates.json")
...
# oneliners
json_oneliners_list <- rjson::fromJSON(file = "json/input_oneliners.json")

# multiliners
json_grouped_list <- rjson::fromJSON(file = "json/input_grouped.json")
...
# oneliners_currency <- json_oneliners_list$currency_oneliners
# oneliners_currency_text <- oneliners_currency

use_oneliners <- unlist(json_oneliners_list[grep("use_oneliners", names(json_oneliners_list), value = T)])
oneliners_currencies <- unlist(json_oneliners_list[grep("currency_oneliners", names(json_oneliners_list), value = T)])
oneliners_currencies_filtered <- oneliners_currencies[use_oneliners]
oneliners_values <- unlist(json_oneliners_list[grep("oneliners_value", names(json_oneliners_list), value = T)])
...
oneliners_texts <- unlist(json_oneliners_list[grep("oneliners_text", names(json_oneliners_list), value = T)])
oneliners_texts_filtered <- oneliners_texts[use_oneliners]

oneliners_exchanges <- unlist(json_oneliners_list[grep("exchange_Final_Oneliners", names(json_oneliners_list), value = T)])
...
# final_to_oneliners_exchange_conditions <- final_currency == oneliners_currency
...
common_plus_sick_pay_rounded <- round(common_plus_sick_pay, 1)
...
sick_pay_rounded <- round(sick_pay, 1)

salary_days_pay_final <- ifelse(use_salary, salary_days_pay_final, 0)
oneliners_final <- 0
...
if (use_grouped) {
  groupedCosts <- unlist(json_grouped_list[grep("groupedCostsValue", names(json_grouped_list), value = T)])

  if (final_to_grouped_exchange_condition) {
    grouped_final <- groupedCosts
  } else {
    grouped_final <- round(groupedCosts / json_grouped_list$exchange_Final_Grouped)
  }

  grouped_items_text <- unlist(json_grouped_list[grep("groupedCostsName", names(json_grouped_list), value = T)])
  ...
  if (!final_to_grouped_exchange_condition) {
    exchange_grouped_text <- paste(
      ...
      "/", json_grouped_list$exchange_Final_Grouped,
      "="
    )
  }
}
...
cat(
  ...
  if (use_dates) {
    paste0("|\\hspace{2mm}", json_salary_list$delivery_period_text, " ", smna, " ", sy, "| | |  |   |   |\n")
  },
  if (use_sick && use_salary) {
    paste0(
      "|\\hspace{8mm}", json_sick_list$text, ": |", sick_days, "/", total_days, "|",
      "|", sick_pay_rounded, "|", salary_currency_text, "|  |", "\n"
    )
  },
  ...
  if (use_grouped) {
    ...
    grouped_strings <- c(
      ...
      grouped_strings,
      paste0(
        "|", json_grouped_list$groupedCostsGeneralName, "| |", grouped_currency_text, "|", exchange_grouped_text, "|", total_grouped_finals_exchanged, " ",
        final_currency, "|", "\n"
      )
    )
    grouped_strings
