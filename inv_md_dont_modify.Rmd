...
smna <- month(params$start_date,label = T, abbr=F)
...
|\hspace{2mm} Delivery date: `r smna` `r sy`                                |
|\hspace{2mm} 1 month: `r exchange`  | `r result`   `r jsonSalaryL$finalCurrency` |
|Remote fees             | `r benefits` `r jsonSalaryL$finalCurrency`             | 
...
cat(paste0("\\hspace{5mm} Name:",    jsonL$intermediary_bank_name,", ",jsonL$intermediary_bank_country,"  \n")  )
```
...
\hspace{5mm} Address: `r jsonL$final_bank_address`, `r jsonL$final_bank_country`  
