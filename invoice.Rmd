...
j3rdL    <- rjson::fromJSON(file = "input_local.json")
...
exchangeDestiny <- paste(j3rdL$localCosts, j3rdL$destinationCurrency
...
result <- ifelse(jSalaryL$use_salary, result, 0)

use_benefits <- jSalaryL$`use_benefits (Remote fees)`

use_salary <- jSalaryL$use_salary
...
localresult <- ifelse(j3rdL$use_local_option, 
                      round(j3rdL$localCosts/mainL$exchangeUSDBRL ),
                      0)

total <- result + benefits + PDB + localresult

result <- format(result, scientific = F)

localresult <- format(localresult, scientific = F)
...
cat(
  ...
  if(j3rdL$use_local_option){
    paste0("|", jFieldsL$localCosts[lan_idx]," ", exchangeDestiny,"|",localresult," ", jSalaryL$finalCurrency,"|","\n")
  },
